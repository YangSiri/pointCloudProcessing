...
namespace
{
#define HDL_Grabber_toRadians(x) ((x) * M_PI / 180.0)//dgree 2 rad

    const int HDL_NUM_ROT_ANGLES = 36001;
    const int HDL_LASER_PER_FIRING = 32;//32bytes data per data_block
    const int HDL_MAX_NUM_LASERS = 64;
    const int HDL_FIRING_PER_PKT = 12;//12 data_blocks per data_packet
...
#pragma pack(pop)
    ...
    struct HDLFiringData
    {
        ...
        unsigned short rotationalPosition;//azimuth per data_block
...
{
public:
    ///one frame = one round(azimuth=360) laser pts?
    struct HDLFrame
    {
        std::vector<double> x;
        std::vector<double> y;
